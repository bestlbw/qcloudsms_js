...
function createQcloudSms(appid, appkey) {
    ...
    //模版模块
    var atemp, mtemp ,dtemp ,gtemp;
    qcloudsms.AddTemplate = function() {
        if (!atemp) {
            atemp = new tem.AddTemplate(qcloudsms.appid, qcloudsms.appkey)
        }
        return atemp;
    };
    qcloudsms.ModTemplate = function() {
        if (!mtemp) {
            mtemp = new tem.ModTemplate(qcloudsms.appid, qcloudsms.appkey)
        }
        return mtemp;
    };
    qcloudsms.DelTemplate = function() {
        if (!dtemp) {
            dtemp = new tem.DelTemplate(qcloudsms.appid, qcloudsms.appkey)
        }
        return dtemp;
    };
    qcloudsms.GetTemplate = function() {
        if (!gtemp) {
            gtemp = new tem.GetTemplate(qcloudsms.appid, qcloudsms.appkey)
        }
        return gtemp;
    };
